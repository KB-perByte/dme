...
class ActionModule(ActionNetworkModule):
    def run(self, tmp=None, task_vars=None):
        ...
        self._config_module = True if module_name in ["dme_nxos_config", "config"] else False
        ...
        if module_name == "dme_nxos_file_copy":
        ...
        if module_name == "dme_nxos_install_os":
            ...
            if persistent_command_timeout < 600 or persistent_connect_timeout < 600:
                ...
                msg += (
                    " must be set to 600 seconds or higher when using dme_nxos_install_os module."
                )
        ...
        if persistent_connection in ("httpapi"):
            if module_name == "dme_nxos_gir":
                conn = Connection(self._connection.socket_path)
                ...
                if persistent_command_timeout < gir_timeout:
                    ...
                    msg = f"timeout value extended to %ss for dme_nxos_gir {gir_timeout}"
                    display.warning(msg)
